...
CREATE USER proveedor_service WITH PASSWORD 'proveedor_password';
CREATE USER product_service WITH PASSWORD 'product_password';
CREATE USER cliente_service WITH PASSWORD 'cliente_password';
...
CREATE DATABASE proveedor_db OWNER proveedor_service;
CREATE DATABASE product_db OWNER product_service;
CREATE DATABASE cliente_db OWNER cliente_service;
...
\c proveedor_db;
GRANT ALL ON SCHEMA public TO proveedor_service;

-- Crear los tipos ENUM necesarios
CREATE TYPE tipo_proveedor_enum AS ENUM ('laboratorio', 'distribuidor', 'importador');
CREATE TYPE pais_enum AS ENUM ('colombia', 'peru', 'ecuador', 'mexico');
CREATE TYPE estado_proveedor_enum AS ENUM ('activo', 'inactivo', 'suspendido');

-- Crear la tabla proveedores en la base de datos proveedor_db
CREATE TABLE proveedores (
    ...
    tiempo_entrega_promedio INTEGER NULL,
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    version INTEGER NOT NULL DEFAULT 1,
    CONSTRAINT calificacion_rango CHECK (calificacion IS NULL OR (calificacion >= 0 AND calificacion <= 5)),
    CONSTRAINT tiempo_entrega_positivo CHECK (tiempo_entrega_promedio IS NULL OR tiempo_entrega_promedio > 0)
);

-- Crear índices para optimizar búsquedas
CREATE INDEX idx_proveedor_nit ON proveedores(nit);
CREATE INDEX idx_proveedor_pais ON proveedores(pais);
CREATE INDEX idx_proveedor_estado ON proveedores(estado);

-- Otorgar permisos DESPUÉS de crear la tabla
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO proveedor_service;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO proveedor_service;

\c cliente_db;
GRANT ALL ON SCHEMA public TO cliente_service;

-- Otorgar permisos DESPUÉS de conectar a cliente_db
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO cliente_service;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO cliente_service;
